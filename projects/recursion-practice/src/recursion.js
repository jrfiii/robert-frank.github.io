...
var range = function(x, y, arr = []) {
  if (y >= x) {
    if (y === x || y === x + 1) return arr;
    arr.push(x + 1);
    return range(x + 1, y, arr);
  }
  if (x >= y) {
    if (x === y || x === y + 1) return arr;
    arr.push(x - 1);
    return range(x - 1, y, arr);
  }
};
...
var exponent = function(base, exp) {
  ...
  if (exp < 0) {return parseFloat(((1 / base) * exponent(base, exp + 1)).toFixed(6));}
};
...
var reverseArr = function (array, revArr = []) {
  if (array.length === 0) return revArr;
  revArr.unshift(array[0]);
  return reverseArr(array.slice(1), revArr);
};
...
var countOccurrence = function(array, value, tally = 0) {
  if (array.length === 0) return tally;
  if (array[0] === value) return countOccurrence(array.slice(1), value, tally + 1);
  else return countOccurrence(array.slice(1), value, tally);
};

// 20. Write a recursive version of map.
// rMap([1,2,3], timesTwo); // [2,4,6]
var rMap = function(array, callback, mapArr = []) {
  if (array.length === 0) return mapArr;
...
var nthFibo = function(n, fibo = [0, 1]) {
  if (n < 0) return null;
  if (n < fibo.length - 1) {return fibo[n];}
  else {
    fibo[fibo.length] = fibo[fibo.length - 2] + fibo[fibo.length - 1];
  }
...
var capitalizeFirst = function(array, output = []) {
  ...
  output.push(array[0].split('')[0].toUpperCase() + array[0].slice(1));
...
var flatten = function (arrays, flat = []) {
  if (arrays.length === 0) {return flat;}
  if(Array.isArray(arrays[0])) {
    flatten(arrays[0], flat);
  } else {
    flat.push(arrays[0]);
  }
  return flatten(arrays.slice(1), flat);
}
...
var compress = function(list, noConsecutiveDupes = []) {
  if (list.length === 0) return noConsecutiveDupes;
  if (noConsecutiveDupes[noConsecutiveDupes.length - 1] !== list[0]) {
    noConsecutiveDupes.push(list[0]);
  }
  return compress(list.slice(1), noConsecutiveDupes);
};
...
var minimizeZeroes = function(array, noRepeatZeroes = []) {
  if (array.length === 0) {
    return noRepeatZeroes;
  }
  if (noRepeatZeroes[noRepeatZeroes.length - 1] !== 0 || array[0] !== 0) {
    noRepeatZeroes.push(array[0]);
  }
  return minimizeZeroes(array.slice(1), noRepeatZeroes);
};
